...
public class ServiceVoiture implements IService<Voiture> {
    ...
    public void update(Voiture voiture) {
        String qry = "UPDATE `voiture` SET `marque`=?, `modele`=?, `annee`=?, `prixParJour`=?, `disponible`=?, `dateDeLocation`=?, `dateDeRemise`=? WHERE `id_voiture`=?";
        try {
    ...
    public Voiture getById(int id) {
        String qry = "SELECT * FROM `voiture` WHERE `id_voiture`=?";
        try {
            PreparedStatement pstm = cnx.prepareStatement(qry);
            pstm.setInt(1, id);
    ...
    public void delete(Voiture voiture) {
        String qry = "DELETE FROM `voiture` WHERE `id_voiture`=?";
        try {
            PreparedStatement pstm = cnx.prepareStatement(qry);
