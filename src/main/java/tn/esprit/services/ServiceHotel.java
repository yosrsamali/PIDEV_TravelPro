...
public class ServiceHotel implements IService<Hotel> {
    ...
    public Hotel getById(int id) {
        String qry = "SELECT * FROM `hotel` WHERE `id_hotel`=?";
        try {
            PreparedStatement pstm = cnx.prepareStatement(qry);
            pstm.setInt(1, id);
    ...
    public void update(Hotel hotel) {
        String qry = "UPDATE `hotel` SET `nom`=?, `ville`=?, `prixParNuit`=?, `disponible`=?, `nombreEtoile`=?, `typeDeChambre`=?, `dateCheckIn`=?, `dateCheckOut`=? WHERE `id_hotel`=?";
        try {
            PreparedStatement pstm = cnx.prepareStatement(qry);
            pstm.setString(1, hotel.getNom());
            pstm.setString(2, hotel.getVille());
    ...
    public void delete(Hotel hotel) {
        String qry = "DELETE FROM `hotel` WHERE `id_hotel`=?";
        try {
            PreparedStatement pstm = cnx.prepareStatement(qry);
